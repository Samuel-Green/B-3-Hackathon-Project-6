...
be scientifically interpreted, most analytical approaches assess
discrete points within the ecosystem, such as those from a quadrat or
transect. A limitation of Shannon's H value is that it does not consider
...
value are prone to saturation, even when only marginal differences are
observed within the objects in a remote sensing dataset.
...
This equation was reproduced from Maus *et al* [@Maus2016].

In addition to the standard cost matrix of the DTW function, they also
propose the equation above to implement a temporal cost. In the equation
α is the steepness of the logistic function which is used to impose a
distance cost on temporal separation, and β is the midpoint of the
curve. Lastly, $g(t_i,t_j)$ represents the time elapsed between the
...
The Rao's Q index with TWDTW distance calculated over a time-series of
...
the study site ([Figure 1](#Figure_1)). The study site is characterized
...
(PPI) covering all images taken during 2023. Each image was 20 pixels
vertically by 27 pixels wide. The PPI index was chosen as it is
...
Phenological Index for every pixel within the Macchia Sacra Special
...
2](#0){style="font-size: 11pt;"} highlights 2 interesting secondary PPI
...
further demonstrated that incorporation of phenological data enhances a
the inferential power of analyses, as incorporation of the temporal
dimension more accurately identified biodiversity hotspots than Rao's Q
