...
of the logistic function which is used to impose a distance cost on
temporal separation, and $\beta$ is the midpoint of the curve, setting a
threshold below which separation in time does not cause a heavy cost on
the metrics. Lastly, $g(t_i,t_j)$ represents the time elapsed between
the dates evaluated in the match ($t_i$ and $t_j$ times of the $i$th and
$j$th observations).
