...
between the dates ($ti$ in the original pattern, and $tj$ in the time
series).
...
`steepness` A numeric value corresponding to the α variable from the
time-weighting function in Maus [@Maus2016]. Lower or higher values of α
...
the unit specified by the `time_scale` argument.
